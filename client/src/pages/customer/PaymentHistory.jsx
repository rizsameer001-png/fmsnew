...
const PaymentHistory = () => {
  ...
  // Safely access payment data with fallbacks
  const paymentList = payments?.payments || [];
  
  const stats = {
    total: paymentList.length,
    totalAmount: paymentList.reduce((sum, p) => sum + (p.status === 'success' ? p.amount : 0), 0),
    successful: paymentList.filter(p => p.status === 'success').length,
  };

  const filteredPayments = paymentList.filter(p => filter === 'all' || p.method === filter);
              ...
              <div className="flex items-start justify-between flex-wrap gap-4">
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">Invoice: {payment.invoiceId?.invoiceNumber || 'N/A'}</p>
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs text-gray-400 mt-1 capitalize">{payment.method || 'Unknown'}</p>
