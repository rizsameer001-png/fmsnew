...
const uploadImageToCloudinary = async (file, userId, type) => {
  if (!file) return null;
  try {
    const result = await uploadToCloudinary(file.buffer, {
      folder: `attendance/${type}/${userId}`,
...
const checkIn = async (req, res) => {
  try {
    ...
    let photoUrl = null;
    if (req.file) {
      photoUrl = await uploadImageToCloudinary(req.file, req.user._id, 'checkin');
    }
    ...
    await ActivityLog.create({
      ...
      details: { location: { latitude, longitude }, lateMinutes: attendance.lateMinutes, photo: photoUrl },
      ipAddress: req.ip
    });
...
const checkOut = async (req, res) => {
  try {
    ...
    let photoUrl = null;
    if (req.file) {
      photoUrl = await uploadImageToCloudinary(req.file, req.user._id, 'checkout');
    }
    ...
    if (req.user.supervisorId) {
      await Notification.create({
        userId: req.user.supervisorId,
        title: 'Attendance Update',
        message: `${req.user.name} (${req.user.role}) checked out at ${attendance.checkOut.time.toLocaleTimeString()}`,
        type: 'attendance',
        data: { userId: req.user._id, attendanceId: attendance._id, photo: photoUrl }
      });
    }

    res.json({ success: true, attendance, photo: photoUrl });
  } catch (error) {
...
const getTodayStatus = async (req, res) => {
  try {
    ...
    res.json({
      ...
      checkInPhoto: attendance?.checkIn?.photo || null,
      checkOutPhoto: attendance?.checkOut?.photo || null
    });
  } catch (error) {
